...
import { Handshake, ArrowUpDown, SortAsc, SortDesc, SearchX, AlertTriangle, Plus } from "lucide-react"
import SearchBar from "../SearchBar"
import TrustedCompanyRow from "./TrustedCompanyRow"
...
export default function TrustedCompanyTable({ companies, loading, error, onRetry, onEdit, onDelete, onAdd }) {
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 2xl:gap-6 2xl:mb-6">
        <SearchBar placeholder="Search companies..." value={query} onChange={setQuery} className="w-full sm:max-w-xs 2xl:max-w-sm" />
      ...
      {loading ? (
        ...
        <EmptyState
          icon={query ? SearchX : Handshake}
          title={query ? "No results found" : "No trusted companies yet"}
          description={
            query
              ? "Try a different search term."
              : "Add the companies you work with to build trust with your visitors."
          }
          action={query ? undefined : { label: "Add Company", icon: Plus, onClick: onAdd }}
        />
      ) : (
        <>
            ...
            <table className="w-full text-sm min-w-[760px]">
              <thead>
                <tr className="text-left text-xs text-gray-400 uppercase tracking-wider border-b border-gray-100">
                  ...
                  <th className="py-2.5 pr-4 2xl:py-3 2xl:pr-6 font-semibold">Website</th>
                  <th className="py-2.5 pr-4 2xl:py-3 2xl:pr-6 font-semibold">
                    <button
                      type="button"
